...
Both of the choices get routed to the attack and decay section. This is important as even if the step does not trigger a sound output it still triggers a value that tells the attack and decay section to remain at zero, this is important as this section knows it has not missed a step and can safely move onto the next step. The attack and decay section works by receiving the previously stated values, triggering the velocity after a specified amount of milliseconds and then going to zero in a selected amount of time, after waiting for the specified attack time to pass. the calculation is as follows: 1 $1, 0 $2 $1. The dollar signs act as variables so an example would be: go to 1 after waiting 100 milliseconds ($1), then go to 0 after waiting for the 100 milliseconds to pass ($2) in 200 millseconds ($1).
